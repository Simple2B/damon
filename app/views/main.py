...
def index():
    ...
    new_table = Assign.query.order_by(Assign.assignID.desc()).paginate(
...
def intransit():
    ...
    page = request.args.get("page", 1, type=int)
    new_table = (
        Assign.query.filter(Assign.Status == "InTransit")
        .order_by(Assign.assignID.desc())
...
def add_record(ticket_id):
    ...
    if form.validate_on_submit():
        ...
        if not elem:
            new = Assign(
                TicketID=ticket_id, Loads=form.loads.data, Status=form.status.data
            )
            new.save()
        else:
